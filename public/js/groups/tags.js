...
function updateTabs(){
    ...
    let tagGroupEls = tagGroups.map(function(gItem, index){
                        ...
                        return `<div class="col-sm-12 col-md-3 mb-3 group_${randNum}" data-groupId="${gItem.id}">
                                    <div class="row m-2">
                                        <div class="col-sm-12 card p-0">
                                            <div class="card-title p-2 text-white bg-primary">
                                                    <i class="fa fa-bars handle"></i>  ${gItem.name}
                                                    <i class="fa fa-edit btn btn-sm pull-right"></i>
                                            </div>
...
function addNewTag(randomNum){
    ...
    if(newTag){
        ...
        let tagPresent = null;
        let sGroup = tagGroups.find(function(item){
                                return   item.id == groupId;     
                    // return item.tags.find(function(tagitem){
                    //             return tagitem.name == newTag;
                    //         })
                        });
        console.log(sGroup);
        if(sGroup){
        ...
        if(tagPresent){
            $(".error_msg_"+randomNum).text("Tag Already Present");
        }
        console.log("newTag presents : ", tagPresent);
        console.log(randomNum);
    }else {
        $(".error_msg_"+randomNum).text("Enter tag Name");
    }
}
