    ...
    let reviewBlk = formData.elObject.map(function (item) {
        ...
        if (item.type == 1) {
            ...
            if (["Text", "Paragraph", "Date", "Number"].includes(item.fieldTitle)) {
                ...
                let optEls = item.options.map(function (optItem) {
                    return `<div class="col-sm-4"><input type="checkbox" data-type="2" data-label="${item.label}" class="field_data form-control" value="${optItem.value}" name="${item.name}" ${$required}>${optItem.option}</input></div>`;
                });
